...
//        c.add(mode);
    ...
    if(evt.getSource() == proceed){
        ...
        boolean ans= true;
        System.out.println(globalSchema.getName());
        try {
            System.out.println(globalSchema.getFact());
            WriteXmlFile writeXmlFile= new WriteXmlFile(globalSchema);
            writeXmlFile.writeSchema();
//           ans =sc.writeSchema(globalSchema);
        ...
        if(ans)
            ...
            System.out.println("Unknown error occurred while schema creation");
